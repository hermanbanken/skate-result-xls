...
function maxAge(count, type) {
	...
	switch (type) {
		case 'w':
			delta = count * 1000 * 3600 * 24 * 7;break;
		case 'd':
	...
	return function (date) {
		return new Date().getTime() - date.getTime() > delta;
	};
}

Array.prototype.distinct = function (key) {
	if (typeof key == 'undefined') key = function key(id) {
		return id;
	};
	var unique = {};
	var distinct = [];
...
function onError(e) {
	...
	this.write(e.stack && e.stack.replace(/\n/g, "\n<br>").replace(/\s/g, "&nbsp;").replace(/\t/g, "&nbsp;&nbsp;&nbsp;&nbsp;") || JSON.stringify(e));
	this.end();
}

var AllCompetitions = rx.Observable.interval(1000 * 3600).startWith(0).flatMap(function (_) {
	return Vantage.competitions().toArray();
}).shareReplay(1);

var Meta = AllCompetitions.take(1).map(function (list) {
	return {
		venues: list.map(function (c) {
			return c.venue;
		}).filter(function (id) {
			return id;
		}).distinct(function (v) {
		...
		disciplines: list.map(function (c) {
			return c.discipline;
		}).distinct()
	};
}).shareReplay(1);
...
module.exports = function (app) {
	...
	app.get('/api/meta', function (req, res) {
		res.type('json');
		Meta.subscribe(function (meta) {
			return res.send(JSON.stringify(meta, undefined, "  "));
		});
	});
	...
	app.get('/api/competitions', function (req, res) {
		...
		function filter(comp) {
			if (req.query.venue && (!comp.venue || comp.venue.code != req.query.venue)) {
				return false;
			}
			if (req.query.discipline && comp.discipline != req.query.discipline) {
				return false;
			}
			if (check(new Date(comp.starts))) {
				return false;
			}
			return true;
		}

		AllCompetitions.take(1).subscribe(function (list) {
	...
	app.get('/api/v2/competitions/:id', function (req, res) {
		Vantage.competition(req.params.id).subscribe(function (c) {
			res.type('json').send(JSON.stringify(c, undefined, "  "));
		}, onError.bind(res));
	});
