...
function fetch(url, options) {
	...
	function retrieve() {
		...
		method(httpOptions, function (response) {
			if (options.dataType == 'binary') response.setEncoding('binary');
		...
		if (typeof options.validate == 'function') return source.promise.then(function (data) {
			if (!options.validate(data)) throw new Error("Invalid response");
			return data;
		});else return source.promise;
	}
	...
	if (options.cache) {
		if (!options.cache.key) options.cache.key = typeof url == 'string' ? url : JSON.stringify(url);
	...
	// Validate cache too	
	if (typeof options.validate == 'function') promise = promise.then(function (data) {
		if (!options.validate(data)) throw new Error("Invalid response");
		return data;
	});

	// Unwrap buffer
	if (options.dataType != 'binary') promise = promise.then(function (buffer) {
